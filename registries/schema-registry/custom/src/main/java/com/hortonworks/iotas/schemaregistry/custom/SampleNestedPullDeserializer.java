 ...
 * under the License.
 */
package com.hortonworks.iotas.schemaregistry.custom;

import com.hortonworks.iotas.common.Schema;
import com.hortonworks.iotas.common.exception.ParserException;
import com.hortonworks.iotas.schemaregistry.serdes.pull.EndFieldContext;
import com.hortonworks.iotas.schemaregistry.serdes.pull.EndRecordContext;
...
import com.hortonworks.iotas.schemaregistry.serdes.pull.StartFieldContext;
...
public class SampleNestedPullDeserializer implements PullDeserializer<SchemaDetails, Schema.Field> {
    ...
    @Override
    public PullEventContext<Schema.Field> next() throws ParserException {
    ...
    private PullEventContext<Schema.Field> processFields() throws ParserException {
        PullEventContext<Schema.Field> context;
        ...
        if (startFieldIndex == endFieldIndex) {
            throw new ParserException("Invalid payload format!!");
        }
        ...
        if (startFieldIndex < endFieldIndex) {
            ...
            if (schemaFields.empty()) {
                ...
                if (!(peekField instanceof Schema.NestedField)) {
                    throw new ParserException("Expected nested field");
                }
                peek.index++;
            ...
            if (!fieldName.equals(currentField.getName())) {
                throw new ParserException("Expected name [" + fieldName + "], encountered unexpected name [" + currentField.getName() + "]");
            }
    ...
    private void handleEndEvent() throws ParserException {
        try {
    ...
    private PullEventContext<Schema.Field> handleStartEvent() throws ParserException {
        rootIndex = 0;
        PullEventContext<Schema.Field> context;
        try {
            if (currentLine == null) {
